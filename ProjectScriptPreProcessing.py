...
def get_hourly_data(layer,folder):
    """
    Prepares daily data from each bird and save them separate shapefiles, each of them containing data for that bird and that particular day.
    ...
    if caps & QgsVectorDataProvider.ChangeAttributeValues:
        ...
        for tag in tags:
            ...
            dates=list()
            for features in selection:
                dates.append(features['trueDate'])
            #Get Unique Values for arbitrary Date
            datelist=getUniqueValuesFromList(dates)
            for date in datelist:
                #outputfilename=tag+"D"+date.replace("-","_")+".csv"
                #outputcsv=os.path.join(folder,outputfilename)
                express=' \"tag_ident\" = \'{tag}\' AND \"arbDate\" = \'{date}\' '.format(tag=tag,date=date)
                processing.run('qgis:selectbyexpression',{ 'EXPRESSION' : express, 'INPUT' : layer, 'METHOD' : 0 })
                #processing.run('native:saveselectedfeatures',{'INPUT':layer,'OUTPUT':outputcsv})
                #print("{output} file written to {folder}".format(output=outputfilename,folder=folder))
                selected_hours = layer.selectedFeatures()
                hours = list()
                for selected_hour in selected_hours:
                    hours.append(selected_hour['Hour'])
                #Get Unique Values for true hour
                hourlists = getUniqueValuesFromList(hours)
                for hourlist in hourlists:
                    hour_csv = os.path.join(folder, 'csv', f'{tag}_{date.replace(" ", "_")}_{hourlist}_hour.csv')
                    #print(tag,date,hourlist)
                    expression = f'tag_ident = {tag} AND trueDate = \'{date}\' AND Hour = \'{hourlist}\' '
                    #expres = 'tag_ident = 5159 AND trueDate = \'2016-06-14\' '
                    #expres = ' \"tag_ident\" = \'{v01}\' AND \"trueDate\" = \'{v02}\' '.format(v01=tag,v02=date)
                    processing.run('qgis:selectbyexpression',{ 'EXPRESSION' : expression, 'INPUT' : layer, 'METHOD' : 0})
                    processing.run('native:saveselectedfeatures',{'INPUT':fileDir,'OUTPUT':hour_csv})
...
get_hourly_data(layer, work_dir)
